...
// components
import NormalSelect from "../../../../../../exhibition/exhibitioninfo/components/NormalSelect";
...
const contentSubtype = [
  { label: "Narrative Feature｜劇情長片  ", value: 0 },
  { label: "Documentary｜紀錄片", value: 1 },
  { label: "Animation｜動畫 ", value: 2 },
  { label: "Short｜短片 ", value: 3 },
  { label: "Restored Classic｜經典修復 ", value: 4 },
  { label: "Immersive Content｜沉浸式內容 ", value: 5 },
];
...
const MovieComponents = ({
  ...
  const [formData, setFormData] = useState({
    contentSubtype: [1], // Type | 類別 (帶1 2 3)    | 必填
    genre: [], // Theme / genre｜主題/類型  | 必填
    originalTitle: "", // Original Title
    status: "", // 製作進度                  | 必填
    expectedCompletionDate: "",
    ExpectedCompletionYear: "",
    ExpectedCompletionMonth: "",
    runningTimeMinutes: 0, // 片長                     | 必填
    descriptionEn: "", // Synopsis                 | 必填
    descriptionZh: "", // 劇情介紹
    ...
    trailerUrl: "", // 預告片連結
    trailerImage: "", // 預告片圖片
    directorNameEn: "", // 導演英文名                   | 必填
    directorNameZh: "", // 導演中文名
    producerNameEn: "", // 製片英文名                   | 必填
    producerNameZh: "", // 製片中文名
    hostCastEn: "", // 卡司英文
    hostCastZh: "", // 卡司中文
  });
  ...
  const t = useTranslations("worksPopup");

  // 必填欄位檢查 props
  const validateFormData = () => {
    const requiredFields = [
      { key: "titleEn", label: "Title" },
      { key: "contentSubtype", label: t("label_1") },
      { key: "genre", label: t("label_2") },
      { key: "status", label: t("label_3") },
      { key: "ExpectedCompletionYear", label: "Year" },
      { key: "ExpectedCompletionMonth", label: "Month" },
      { key: "runningTimeMinutes", label: t("label_5") },
      { key: "descriptionEn", label: t("label_6") },
      { key: "directorNameEn", label: "Director's Name" },
      { key: "producerNameEn", label: "Producer's Name" },
      { key: "hostCastEn", label: "Host/Cast" },
    ];
  ...
  useEffect(() => {
    validateFormData();
    // console.log("Movie form validation:", isValid);
  }, [formData]);
  ...
  useEffect(() => {
    if (work) {
      ...
      const updatedWork = {
        ...
        // contentSubtype: work?.contentSubtype[0] ? work.contentSubtype : '',
      };
  ...
  const p = useTranslations("MoviePlaceholder");

  const handleChange = (name, value) => {
    if (name === "runningTimeMinutes") {
      ...
      <div className={styles.television__mainWrapper}>
        <div className={styles.television__gridBlock}>
          ...
          <MultipleSelect
            ...
            onChange={(selectedOptions) =>
              handleChange("genre", selectedOptions)
            }
            defaultText={p("theme")}
            require={true}
          />
        ...
        <div className={styles.television__textAreaBlock}>
          <InputComponent
            ...
            message="Up to 600 characters, including punctuation and spaces.<br/>If the character limit is exceeded, the text will be edited as necessary without notification."
            maxLength={600}
        ...
        <div className={styles.television__mixedInputBlock}>
          <div className={styles.television__gridBlock}>
            ...
            <InputComponent
              type="text"
              label="Key Cast"
              placeholder="please enter key cast name"
              message={
                "first name, LAST NAME<br/>Up to three Host / Key Cast names, separated by slashes.<br/>Example:first name A, LAST NAME A/first name B,LAST NAME B/first name C,LAST NAME C"
              }
              name="hostCastEn"
